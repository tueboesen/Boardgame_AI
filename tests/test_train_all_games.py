...
from src.players import PlayerTemplate, PLAYMODES, MCTSNN
...
def build_game_components(game_ref):
    ...
    if key in game_ref:
        ...
        gc['mcts'] = mcts
        if len(game_ref[key]) > 3:
            coach_args = game_ref[key][3]
        else:
            coach_args = nn_args
        gc['coach'] = coach_args
...
def build_trainable_games():
    for game_ref in GAMES_AVAILABLE:
        game_components = build_game_components(game_ref)
        available_players = determine_available_players(game_components,playmodes=MCTSNN)
        for player in available_players:
            viz = game_components['viz'] if 'viz' in game_components else None
            yield player, game_components['game'], game_components['coach'], viz


@pytest.mark.parametrize("players,game,coach_args,viz", build_trainable_games(), ids=str)
def test_train_all_games(player, game, coach_args, viz):
    c = Coach(game,player,coach_args)
    c.learn()
